...
Examples are stored with vector embeddings generated by `text-embedding-3-small`. When classifying a new document, the system embeds the document text and retrieves the top-3 most semantically similar examples using cosine similarity via `pgvector`.

This means only relevant examples are injected — a capital call document retrieves capital call corrections, not invoice corrections. It also prevents token limit issues as the Examples table grows, and avoids injecting conflicting examples from unrelated document types.
